...
describe("ColorPanel.test.mjs", () => {
  it("should render component", () => {
    //when
    const result = TestRenderer.create(h(ColorPanel)).root;

    //then
    assert.deepEqual(ColorPanel.displayName, "ColorPanel");
    assertColorPanel(result);
  });
});
 ...
 */
function assertColorPanel(result) {
  assertComponents(
    result.children,
    h("log", {
      autoFocus: false,
